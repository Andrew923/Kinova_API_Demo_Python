...
class XboxController:
    def __init__(self, robot_connection):
        ...
        # Control parameters
        self.linear_sensitivity = 0.4 
        self.angular_sensitivity = 0.5
    ...
    def run(self):
        ...
        while self.running:
            ...
            lx = self._apply_deadzone(axes[0])
            ly = self._apply_deadzone(axes[1])
            lt = (axes[4] + 1) / 2  # Left trigger [0, 1]
            rt = (axes[5] + 1) / 2  # Right trigger [0, 1]
            lz = rt - lt  # Combined Z axis

            # --- Rotation Controls ---
            rx = self._apply_deadzone(axes[2])
            ry = self._apply_deadzone(axes[3])
            lb = buttons[4]
            rb = buttons[5]
            rz = rb - lb
